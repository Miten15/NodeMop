...
func Scan(root string, oldAfter time.Duration) ([]project.Project, error) {
	...
	if stat, err := os.Stat(root); err != nil || !stat.IsDir() {
		return nil, errors.New("scan path is not a directory")
	}
	...
	sort.Slice(projects, func(i, j int) bool {
		return projects[i].LastActivity.Before(projects[j].LastActivity)
	})
	return projects, nil
}
...
func inspect(path string, oldAfter time.Duration) project.Project {
	...
	if inside, gitRoot, worktree := gitx.RepositoryInfo(path); inside {
		...
		p.Branch, p.ChangedFiles, p.Ahead, p.Behind, p.GitState = gitx.Inspect(path)
		p.HasUncommitted = p.ChangedFiles > 0
	} else {
		p.LastActivity = newestMeaningfulFile(path)
		p.GitState = project.GitNone
	}

	p.SizeBytes, p.NodeModules = directorySizes(path)
	p.CleanupBytes = cleanup.ReclaimableSize(path)

	if !p.HasGit {
		p.Status = project.StatusNoGit
	} else if !p.LastActivity.IsZero() && time.Since(p.LastActivity) <= oldAfter {
		p.Status = project.StatusActive
	} else {
		p.Status = project.StatusOld
	}
	return p
}
...
func gitLastCommit(path string) time.Time {
	cmd := exec.Command("git", "-C", path, "log", "-1", "--format=%cI")
	out, err := cmd.Output()
	if err != nil {
		return newestMeaningfulFile(path)
	}
...
func directorySizes(root string) (total int64, nodeModules int64) {
	_ = filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return nil
		}
		...
		for _, part := range parts {
			if part == "node_modules" {
				nodeModules += info.Size()
				break
			}
		}
		return nil
	})
	return total, nodeModules
}

// ReadGitignore is intentionally kept small for v0.1; it will be useful when
// the archive engine lands in the next milestone.
